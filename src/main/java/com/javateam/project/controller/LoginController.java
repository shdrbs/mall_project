...
public class LoginController {
	...
	public String loginProc(@RequestParam("id") String id,
		...
		String member = memberService.isActiveMember(id);
		...
		if (member == null) {
			...
			if (msg.equals("아이디/패쓰워드가 모두 일치합니다.") && !member.equals("n")) {
			...
			} else if (msg.equals("패쓰워드가 일치하지 않습니다.") && !member.equals("n")) {

				ra.addFlashAttribute("login_err_msg", msg);
				// 로그인 페이지 이동
				page = "redirect:/login/login_form";

			} else { // 탈퇴한 회원이 로그인할 경우
